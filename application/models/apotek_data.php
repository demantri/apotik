...
class Apotek_data extends CI_Model
{
    ...
    {
        $this->db->select('*');
        $this->db->from('table_sup');
        $this->db->order_by('nama_pemasok');
        $query = $this->db->get();
        return $query;
    }
    ...
    {
        $sql ="SELECT a.*, nm_akun, header_akun, saldo_awal
        FROM jurnal a
        JOIN akun b ON a.no_coa = b.kd_akun
        WHERE kd_akun = '$no_coa' 
        AND LEFT(tgl_input, 7) = '$tgl'
        ";
        return $this->db->query($sql);
    }
